...
app.use(cors({
    origin: ['http://localhost:4200','https://ananta-villa-2024.web.app','https://ananta-villa-2024.firebaseapp.com',],
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        // Get content type and create base64
        ...
        console.log("Photo URL preview:", user.photo.substring(0, 100));
        res.status(200).json({ message: 'Login successful', user: user });
    } catch (error) {
        console.error("Token Decoding Error: ", error);
        res.status(500).json({ message: 'Error decoding token', error: error.message });
    }
});
